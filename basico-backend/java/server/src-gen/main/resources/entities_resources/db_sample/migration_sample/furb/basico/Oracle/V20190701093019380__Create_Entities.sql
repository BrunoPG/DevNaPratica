/* Database: Oracle. Generation date: 2019-07-01 09:30:19:380 */
/* Entity Cliente */
...
alter table pedido add constraint fkf2clv1tugbzcrc3b3ay863a5mefl foreign key (cliente) references cliente (id);
alter table pedido_itens add constraint fklqsvvytaddwinvrkqf2tps86onjo foreign key (pedido_id) references pedido (id);
alter table pedido_itens add constraint fkmghewwnxf58jgu7kp1xgu1wzzm20 foreign key (itens_id) references item (id);
alter table evento add constraint fkadqlc2aliaygaipflqsu6dzwhzwo foreign key (tipo) references tipo (id);
alter table sala_setores add constraint fkoqbauwkehg2iyyjgdzk2cqlrzyiw foreign key (sala_id) references sala (id);
alter table sala_setores add constraint fk5otghwndwk8knglgwen21cf4hubf foreign key (setores_id) references setor (id);
alter table sala_evento add constraint fkyrfw4frunomgevbbbj2ftpng2awj foreign key (sala_id) references sala (id);
alter table sala_evento add constraint fk5id06fddatdy8bddarft24x9rpuk foreign key (evento_id) references evento (id);
alter table setor add constraint fkvvt3zik3rkjt6tsujemrk5yh7tvb foreign key (sala) references sala (id);
