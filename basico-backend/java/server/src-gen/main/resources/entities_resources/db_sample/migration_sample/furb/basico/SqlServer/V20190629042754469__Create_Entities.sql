/* Database: SqlServer. Generation date: 2019-06-29 04:27:54:469 */
/* Entity Cliente */
...
alter table pedido add constraint fklkpwq7idrjsysmdhddwoowigwajw foreign key (cliente) references cliente (id);
alter table pedido_itens add constraint fk82edompuvpafo3kxeq7pmw3kifwk foreign key (pedido_id) references pedido (id);
alter table pedido_itens add constraint fkyricmsplfliddvujgybrtrz3ntjz foreign key (itens_id) references item (id);
alter table evento add constraint fks2zwgfnocmt3o6cfst7wtuvroy13 foreign key (tipo) references tipo (id);
alter table sala_setores add constraint fkvrlim7fkafyhj1mjbsbrmourpury foreign key (sala_id) references sala (id);
alter table sala_setores add constraint fkawsbhnuqrmbeyhslr2hfyod7oysy foreign key (setores_id) references setor (id);
alter table sala_evento add constraint fkjkl3butziatf23x49toaehvxqav9 foreign key (sala_id) references sala (id);
alter table sala_evento add constraint fkbysjy4dk3pbvjftubs7ygxvo9xi4 foreign key (evento_id) references evento (id);
alter table setor add constraint fk9y4gc6wxozgw7s9dw5mprnrkng28 foreign key (sala) references sala (id);
