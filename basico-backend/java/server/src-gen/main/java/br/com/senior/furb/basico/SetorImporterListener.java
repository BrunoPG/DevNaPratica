...
public class SetorImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(SetorImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  BasicoConstants.Events.IMPORT_SETOR_EVENT, new ImportSetorEventPayload(status));
		}
	}
