...
public class EventoImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(EventoImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  BasicoConstants.Events.IMPORT_EVENTO_EVENT, new ImportEventoEventPayload(status));
		}
	}
