...
import static br.com.senior.furb.basico.SalaEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Sala.PageRequest;
import br.com.senior.furb.basico.Sala.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateSala;
import br.com.senior.furb.basico.UpdateSala;
import br.com.senior.furb.basico.DeleteSala;
import br.com.senior.furb.basico.RetrieveSala;
import br.com.senior.furb.basico.ListSala;
...
import br.com.senior.furb.basico.SalaDTOConverter;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = BasicoConstants.class)
@Component("furb.basico.SalaHandler")
public class SalaHandler implements CreateSala, CreateMergeSala, UpdateSala, DeleteSala, 
		 UpdateMergeSala, CreateBulkSala, RetrieveSala, ListSala, 
		 ImportSala, ExportSala {
	static final Logger logger = LoggerFactory.getLogger(SalaHandler.class);

	@Inject
	SalaCrudService service;
	
	@Inject
	SalaDTOConverter dtoConverter;
	...
	public Sala createSala(Sala toCreate) {
		try {
			SalaEntity entity = service.createSala(dtoConverter.toEntity(toCreate, SalaEntity.class));
			return dtoConverter.toDTO(entity, Sala.class);
	...
	@Transactional
	@Override
	public Sala createMergeSala(Sala toCreate) {
		try {
			SalaEntity entity = service.createSala(dtoConverter.toEntity(toCreate, SalaEntity.class));
			return dtoConverter.toDTO(entity, Sala.class);
	...
	public Sala updateSala(Sala toUpdate) {
		try {
			SalaEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Sala.class);
	...
	@Transactional
	@Override
	public Sala updateMergeSala(Sala toUpdate) {
		try {
			SalaEntity entity = service.retrieveSala(new Sala.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateSala(entity);
			return dtoConverter.toDTO(entity, Sala.class);
	...
	private SalaEntity doUpdate(Sala toUpdate) {
		SalaEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Sala_notFound", "Sala not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private SalaEntity doUpdate(SalaEntity entity) {
		return service.updateSala(entity);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_DELETE)
	@Override
	public void deleteSala(Id id) {
		this.service.deleteSala(id);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_VIEW)
	@Transactional(readOnly = true)
	@Override
	public Sala retrieveSala(Id id) {
		SalaEntity entity = service.retrieveSala(id);
		return dtoConverter.toDTO(entity, Sala.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listSala(PageRequest pageRequest) {
		...
		Page<SalaEntity> pages = this.service.listSalaPageable(skip, top, orderBy, filter);
		List<SalaEntity> entities = pages.getContent();
		List<Sala> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Sala.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkSalaOutput createBulkSala(CreateBulkSalaInput toCreate) {
		if (toCreate != null) {
			List<Sala> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<SalaEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, SalaEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkSalaOutput();
	}
	...
	public ImportSalaOutput importSala(ImportSalaInput request) {
		...
		ImportSalaOutput output = new ImportSalaOutput(importer.getId());
		return output;
	}
	...
	public ExportSalaOutput exportSala(ExportSalaInput request) {
		 ...
		 */
		ExportSalaOutput output = new ExportSalaOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new SalaImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Sala.class.getName());
					importer.setCrudService(SalaHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("sala", "blob");
