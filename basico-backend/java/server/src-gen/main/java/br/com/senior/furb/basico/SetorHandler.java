...
import static br.com.senior.furb.basico.SetorEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Setor.PageRequest;
import br.com.senior.furb.basico.Setor.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateSetor;
import br.com.senior.furb.basico.UpdateSetor;
import br.com.senior.furb.basico.DeleteSetor;
import br.com.senior.furb.basico.RetrieveSetor;
import br.com.senior.furb.basico.ListSetor;
...
import br.com.senior.furb.basico.SetorDTOConverter;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = BasicoConstants.class)
@Component("furb.basico.SetorHandler")
public class SetorHandler implements CreateSetor, CreateMergeSetor, UpdateSetor, DeleteSetor, 
		 UpdateMergeSetor, CreateBulkSetor, RetrieveSetor, ListSetor, 
		 ImportSetor, ExportSetor {
	static final Logger logger = LoggerFactory.getLogger(SetorHandler.class);

	@Inject
	SetorCrudService service;
	
	@Inject
	SetorDTOConverter dtoConverter;
	...
	public Setor createSetor(Setor toCreate) {
		try {
			SetorEntity entity = service.createSetor(dtoConverter.toEntity(toCreate, SetorEntity.class));
			return dtoConverter.toDTO(entity, Setor.class);
	...
	@Transactional
	@Override
	public Setor createMergeSetor(Setor toCreate) {
		try {
			SetorEntity entity = service.createSetor(dtoConverter.toEntity(toCreate, SetorEntity.class));
			return dtoConverter.toDTO(entity, Setor.class);
	...
	public Setor updateSetor(Setor toUpdate) {
		try {
			SetorEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Setor.class);
	...
	@Transactional
	@Override
	public Setor updateMergeSetor(Setor toUpdate) {
		try {
			SetorEntity entity = service.retrieveSetor(new Setor.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateSetor(entity);
			return dtoConverter.toDTO(entity, Setor.class);
	...
	private SetorEntity doUpdate(Setor toUpdate) {
		SetorEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Setor_notFound", "Setor not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private SetorEntity doUpdate(SetorEntity entity) {
		return service.updateSetor(entity);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_DELETE)
	@Override
	public void deleteSetor(Id id) {
		this.service.deleteSetor(id);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_VIEW)
	@Transactional(readOnly = true)
	@Override
	public Setor retrieveSetor(Id id) {
		SetorEntity entity = service.retrieveSetor(id);
		return dtoConverter.toDTO(entity, Setor.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listSetor(PageRequest pageRequest) {
		...
		Page<SetorEntity> pages = this.service.listSetorPageable(skip, top, orderBy, filter);
		List<SetorEntity> entities = pages.getContent();
		List<Setor> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Setor.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkSetorOutput createBulkSetor(CreateBulkSetorInput toCreate) {
		if (toCreate != null) {
			List<Setor> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<SetorEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, SetorEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkSetorOutput();
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_CREATE)
	@Transactional
	@Override
	public ImportSetorOutput importSetor(ImportSetorInput request) {
		...
		ImportSetorOutput output = new ImportSetorOutput(importer.getId());
		return output;
	}
	...
	public ExportSetorOutput exportSetor(ExportSetorInput request) {
		 ...
		 */
		ExportSetorOutput output = new ExportSetorOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new SetorImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Setor.class.getName());
					importer.setCrudService(SetorHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("setor", "blob");
