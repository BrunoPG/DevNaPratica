...
public class SalaImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(SalaImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  BasicoConstants.Events.IMPORT_SALA_EVENT, new ImportSalaEventPayload(status));
		}
	}
