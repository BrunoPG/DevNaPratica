...
public class PedidoDTOConverter extends EntityDTOConverter {
	...
	public void setupMapperToEntity(ModelMapper mapper, ConversionContext conversionContext) {
		...
		Converter <List<Item>, List<ItemEntity>> itensConverter = new ContextualizedConverter<List<Item>, List<ItemEntity>>() {
			@Override
			public List<ItemEntity> convert(List<Item> source) {
				if (source == null) {
					return Collections.emptyList();
				}
				return source.stream().map(d -> toEntity(d, ItemEntity.class, conversionContext)).collect(Collectors.toList());
			}
		};
		...
		if (mapper.getTypeMap(Item.class, ItemEntity.class) == null) {
			this.itemConverter.setupMapperToEntity(mapper, conversionContext);
		}
	...
	public void setupMapperToDTO(ModelMapper mapper, ConversionContext conversionContext) {
		...
		Converter <List<ItemEntity>, List<Item>> itensConverter = new ContextualizedConverter<List<ItemEntity>, List<Item>>() {
			@Override
			public List<Item> convert(List<ItemEntity> source) {
				if (conversionContext.isIncludeTranslations()) {
					return source.stream().map(e -> toDTOWithTranslations(e, Item.class, conversionContext.getRelationshipFields("itens"), conversionContext)).collect(Collectors.toList());
				}
				return source.stream().map(e -> toDTO(e, Item.class, conversionContext.getRelationshipFields("itens"), conversionContext)).collect(Collectors.toList());
			}
		};
		...
		if (mapper.getTypeMap(ItemEntity.class, Item.class) == null) {
		    this.itemConverter.setupMapperToDTO(mapper, conversionContext);
		}
