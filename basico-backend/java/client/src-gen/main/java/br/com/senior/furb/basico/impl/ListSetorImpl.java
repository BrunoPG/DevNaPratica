...
public class ListSetorImpl {
	...
	public ListSetorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Setor.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_SETOR, //
	...
	*/
	public Setor.PagedResults listSetor(Setor.PageRequest input, long timeout) {
		...
		Setor.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Setor.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Setor.PagedResults> listSetorRequest(Setor.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Setor.PagedResults.class);
	}
