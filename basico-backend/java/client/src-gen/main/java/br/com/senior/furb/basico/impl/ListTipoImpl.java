...
public class ListTipoImpl {
	...
	public ListTipoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Tipo.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_TIPO, //
	...
	*/
	public Tipo.PagedResults listTipo(Tipo.PageRequest input, long timeout) {
		...
		Tipo.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Tipo.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Tipo.PagedResults> listTipoRequest(Tipo.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Tipo.PagedResults.class);
	}
