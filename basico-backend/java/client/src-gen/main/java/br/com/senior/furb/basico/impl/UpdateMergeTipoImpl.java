 ...
 * 
 */
public class UpdateMergeTipoImpl {
	...
	public UpdateMergeTipoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Tipo input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_TIPO, //
	...
	*/
	public Tipo updateMergeTipo(Tipo input, long timeout) {
		...
		Tipo output = DtoJsonConverter.toDTO(resultMessage.getContent(), Tipo.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Tipo> updateMergeTipoRequest(Tipo input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Tipo.class);
	}
