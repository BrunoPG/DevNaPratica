...
public class ImportEventoImpl {
	...
	public ImportEventoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportEventoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_EVENTO, //
	...
	*/
	public ImportEventoOutput importEvento(ImportEventoInput input, long timeout) {
		...
		ImportEventoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportEventoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportEventoOutput> importEventoRequest(ImportEventoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportEventoOutput.class);
	}
