...
public class ImportItemImpl {
	...
	public ImportItemImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportItemInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_ITEM, //
	...
	*/
	public ImportItemOutput importItem(ImportItemInput input, long timeout) {
		...
		ImportItemOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportItemOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportItemOutput> importItemRequest(ImportItemInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportItemOutput.class);
	}
