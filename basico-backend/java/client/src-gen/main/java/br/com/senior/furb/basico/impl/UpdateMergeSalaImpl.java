 ...
 * 
 */
public class UpdateMergeSalaImpl {
	...
	public UpdateMergeSalaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Sala input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_SALA, //
	...
	*/
	public Sala updateMergeSala(Sala input, long timeout) {
		...
		Sala output = DtoJsonConverter.toDTO(resultMessage.getContent(), Sala.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Sala> updateMergeSalaRequest(Sala input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Sala.class);
	}
