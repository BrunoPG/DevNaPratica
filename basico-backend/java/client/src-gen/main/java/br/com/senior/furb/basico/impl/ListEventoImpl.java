...
public class ListEventoImpl {
	...
	public ListEventoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Evento.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_EVENTO, //
	...
	*/
	public Evento.PagedResults listEvento(Evento.PageRequest input, long timeout) {
		...
		Evento.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Evento.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Evento.PagedResults> listEventoRequest(Evento.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Evento.PagedResults.class);
	}
