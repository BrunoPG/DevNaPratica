...
public class ListSalaImpl {
	...
	public ListSalaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Sala.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_SALA, //
	...
	*/
	public Sala.PagedResults listSala(Sala.PageRequest input, long timeout) {
		...
		Sala.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Sala.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Sala.PagedResults> listSalaRequest(Sala.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Sala.PagedResults.class);
	}
