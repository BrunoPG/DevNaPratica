...
public class ExportTipoImpl {
	...
	public ExportTipoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportTipoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_TIPO, //
	...
	*/
	public ExportTipoOutput exportTipo(ExportTipoInput input, long timeout) {
		...
		ExportTipoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportTipoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportTipoOutput> exportTipoRequest(ExportTipoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportTipoOutput.class);
	}
