 ...
 * 
 */
public class CreateMergeSetorImpl {
	...
	public CreateMergeSetorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Setor input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_SETOR, //
	...
	*/
	public Setor createMergeSetor(Setor input, long timeout) {
		...
		Setor output = DtoJsonConverter.toDTO(resultMessage.getContent(), Setor.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Setor> createMergeSetorRequest(Setor input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Setor.class);
	}
