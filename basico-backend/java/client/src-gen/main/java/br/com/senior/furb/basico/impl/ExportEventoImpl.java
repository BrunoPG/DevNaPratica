...
public class ExportEventoImpl {
	...
	public ExportEventoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportEventoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_EVENTO, //
	...
	*/
	public ExportEventoOutput exportEvento(ExportEventoInput input, long timeout) {
		...
		ExportEventoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportEventoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportEventoOutput> exportEventoRequest(ExportEventoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportEventoOutput.class);
	}
