...
public class ImportSalaImpl {
	...
	public ImportSalaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportSalaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_SALA, //
	...
	*/
	public ImportSalaOutput importSala(ImportSalaInput input, long timeout) {
		...
		ImportSalaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportSalaOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportSalaOutput> importSalaRequest(ImportSalaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportSalaOutput.class);
	}
