...
public class ImportSetorImpl {
	...
	public ImportSetorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportSetorInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_SETOR, //
	...
	*/
	public ImportSetorOutput importSetor(ImportSetorInput input, long timeout) {
		...
		ImportSetorOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportSetorOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportSetorOutput> importSetorRequest(ImportSetorInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportSetorOutput.class);
	}
